...
export default function AdminCategoriesPage() {
  ...
  return (
    <>
      <h2 className="font-display text-3xl font-extrabold text-text-primary mb-8">Categories</h2>
      <div className="grid grid-cols-1 xl:grid-cols-[400px_1fr] gap-8 items-start">
        <div className="bg-card border border-border rounded-3xl p-6 sm:p-8 shadow-sm">
          <h3 className="font-display text-xl font-bold text-text-primary mb-6 border-b border-border/50 pb-4">{editId ? 'Edit Category' : 'Add Category'}</h3>
          <form onSubmit={handleSubmit}>
            <div className="flex flex-col gap-2 mb-5"><label className="font-medium text-text-primary text-sm">Name</label><input className="w-full px-4 py-3 rounded-xl border border-border bg-background text-text-primary focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all" value={name} onChange={e => setName(e.target.value)} required /></div>
            <div className="flex flex-col gap-2 mb-5"><label className="font-medium text-text-primary text-sm">Logo URL</label><input className="w-full px-4 py-3 rounded-xl border border-border bg-background text-text-primary focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all" value={logo} onChange={e => setLogo(e.target.value)} placeholder="https://..." /></div>
            <div className="flex flex-col gap-2 mb-5"><label className="font-medium text-text-primary text-sm">Or Upload Logo</label><input className="w-full px-4 py-3 rounded-xl border border-border bg-background text-text-primary focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all text-sm file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-bold file:bg-primary/10 file:text-primary hover:file:bg-primary/20 cursor-pointer" type="file" accept="image/*" onChange={e => setLogoFile(e.target.files[0])} /></div>
            <button className="w-full inline-flex items-center justify-center font-bold transition-all duration-300 rounded-xl px-8 py-4 bg-primary text-white hover:bg-primary-dark shadow-md mt-4 disabled:opacity-50 disabled:cursor-not-allowed" disabled={saving}>
              {saving ? 'Saving...' : editId ? 'Update' : 'Add Category'}
            </button>
            {editId && <button type="button" className="w-full inline-flex items-center justify-center font-bold transition-all duration-300 rounded-xl px-4 py-4 bg-transparent border-2 border-border text-text-secondary hover:bg-border mt-3" onClick={resetForm}>Cancel</button>}
          </form>
        </div>

        <div className="bg-card border border-border rounded-3xl p-6 sm:p-8 shadow-sm">
          <h3 className="font-display text-xl font-bold text-text-primary mb-6 border-b border-border/50 pb-4">All Categories ({categories.length})</h3>
          <div className="w-full overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[500px]">
              <thead><tr><th className="px-6 py-4 border-b border-border bg-primary-light/5 font-bold text-text-primary text-sm uppercase tracking-wider">Logo</th><th className="px-6 py-4 border-b border-border bg-primary-light/5 font-bold text-text-primary text-sm uppercase tracking-wider">Name</th><th className="px-6 py-4 border-b border-border bg-primary-light/5 font-bold text-text-primary text-sm uppercase tracking-wider"></th></tr></thead>
              <tbody>
                {categories.map(c => (
                  <tr key={c.id} className="hover:bg-background/50 transition-colors border-b border-border last:border-0">
                    <td className="px-6 py-4">{c.logo && <img src={c.logo} className="w-10 h-10 object-contain rounded-md bg-primary-light/10" alt="" />}</td>
                    <td className="px-6 py-4"><strong className="text-text-primary">{c.name}</strong></td>
                ...
                {categories.length === 0 && <tr><td colSpan="3" className="px-6 py-8 text-text-secondary text-sm text-center italic">No categories</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
